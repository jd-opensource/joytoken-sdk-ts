 ...
 */
export function fileRead(sandbox: FileSandbox): Tool {
  return defineTool({
    name: "file_read",
    description:
      "Read a UTF-8 text file located inside the configured directory. The path must be relative to that directory.",
    parameters: {
      type: "object",
      properties: {
        path: {
 ...
 */
export function fileWrite(sandbox: FileSandbox): Tool {
  return defineTool({
    name: "file_write",
    description:
      "Write a UTF-8 text file inside the configured directory, creating parent folders as needed. The path must be relative to that directory.",
    parameters: {
      type: "object",
      properties: {
        path: {
 ...
 */
export function listDir(sandbox: FileSandbox): Tool {
  return defineTool({
    name: "list_dir",
    description:
      'List the entries of a directory inside the configured directory. Use "." for the root. The path must be relative.',
    parameters: {
      type: "object",
      properties: {
        path: {
 ...
 */
export function fileSearch(sandbox: FileSandbox): Tool {
  return defineTool({
    name: "file_search",
    description:
      "Recursively search the configured directory for files whose name matches a glob pattern, e.g. \"*.md\".",
    parameters: {
      type: "object",
      properties: {
        pattern: {
 ...
 */
export function absRoot(sandbox: FileSandbox): string {
 ...
 */
async function resolvePath(sandbox: FileSandbox, rel: string): Promise<string> {
  if (isAbsolute(rel)) {
 ...
 */
async function evalDeepestExisting(abs: string): Promise<string> {
  let remaining = "";
  let current = abs;
  for (;;) {
    try {
...
function maxBytes(sandbox: FileSandbox): number {
