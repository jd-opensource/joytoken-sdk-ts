...
export function messageRequestToChat(request, tools) {
    const messages = [];
    const system = blockText(request.system);
    if (system)
        messages.push({ role: "system", content: system });
    for (const message of request.messages)
        appendAnthropicInput(messages, message.role, message.content);
    const toolChoice = messageToolChoiceToChat(request.tool_choice);
    const { model, max_tokens: maxTokens, temperature, top_p: topP, stop_sequences: stopSequences, tier, metadata, messages: _messages, system: _system, stream: _stream, tools: _tools, tool_choice: _toolChoice, ...extra } = request;
    return {
        ...extra,
        model,
        messages,
...
export function chatResponseToMessage(response) {
...
function appendAnthropicInput(messages, role, content) {
...
function messageToolChoiceToChat(value) {
    if (!value)
    ...
    if (value.type === "tool")
        return { choice: { type: "function", function: { name: value.name } }, parallel };
