...
import type { AgentOptions, AgentTool, ToolExecutionContext } from "../types.js";
import type { Middleware, ToolExecuteFunc } from "./middleware.js";
import { Permission, PermissionMode, permissionMiddleware } from "./permission.js";
...
export class Toolkit {
   ...
   */
  private wrap(name: string, execute: ToolExecuteFunc): ToolExecuteFunc {
    let handler = execute;
 ...
 */
function normalizeExecute(tool: AgentTool): ToolExecuteFunc {
  return (input: unknown, context: ToolExecutionContext) => Promise.resolve(tool.execute(input, context));
}
 ...
 */
export function defaultToolkit(options?: ToolkitOptions): Toolkit {
 ...
 */
export function withDefaults(options: AgentOptions, toolkitOptions?: ToolkitOptions): AgentOptions {
 ...
 */
export function createAgent(options: AgentOptions, toolkitOptions?: ToolkitOptions): Agent {
